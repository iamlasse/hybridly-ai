...
<template>
  <div
    class="kanban-board h-full sm:pb-12 px-4 flex overflow-x-auto text-slate-500"
    @click="handleOutsideClick"
  >
    <draggable
      v-model="computedColumns"
      :animation="200"
    ...
    >
      <template #item="{ element: column }">
        <div
          class="kanban-column flex-shrink-0 w-64 bg-gray-100 p-4 mr-4 rounded flex flex-col"
        >
          <div
            class="flex justify-between items-center mb-4 column-handle cursor-move"
          >
          ...
          <div class="flex-grow overflow-y-auto">
            ...
            >
              <template #item="{ element }">
                <TaskCard
                  :task="element"
                  :selected="element.id === selectedTask?.id"
          ...
          <div class="mt-4">
            <div
              v-if="column.showAddTask"
              class="add-task-card bg-white shadow-sm p-1"
              @click.stop
            >
              <TextInput
                v-model="column.newTaskName"
                type="text"
                placeholder="Write a task name"
                class="w-full mb-2 border-0 active:border-0 ring-0 active:ring-0 focus:ring-0 shadow-none"
                @keyup.enter="addTask(column)"
                @keyup.esc="cancelAddTask(column)"
                ref="newTaskInput"
              />
              <div class="flex justify-end">
                <button
                  @click="cancelAddTask(column)"
            ...
            <Button
              variant="primary"
              v-else
              @click.stop="showAddTaskCard(column)"
              class="w-full"
            >
              + Add task
            </Button>
    ...
    <div
      class="kanban-column-new flex-shrink-0 w-64 bg-gray-100 p-4 mr-4 rounded"
    >
      <Button
      ...
      >
        <input
          v-model="addColumnForm.fields.name"
          type="text"
